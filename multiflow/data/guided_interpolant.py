...
class GuidedInterpolant:
    ...
    def optimize_logits(self, base_logits, reset_theta=False):
        ...
        """
        if self.reward_fn is None or self.guidance_config is None:
            return base_logits
        
        base_logits = base_logits.detach()
        with torch.inference_mode(False):
            gamma = self.guidance_config.get('gamma', 0.1)
            lambda_kl = self.guidance_config.get('lambda_kl', 0.01)
            steps = self.guidance_config.get('steps', 5)
            num_samples = self.guidance_config.get('N', 16)
            theta_clamp = self.guidance_config.get('theta_clamp', None)

            B, N, D = base_logits.shape
            device = base_logits.device

            if (reset_theta or 
                self.last_theta is None or 
